...
dbContainer() {
    vault kv put secret/"${P_NAME}" POSTGRES_PASSWORD="${PG_PASS}"
    POST_PASS=$(vault kv get -field=POSTGRES_PASSWORD secret/"${P_NAME}")

    docker run -d \
    -e POSTGRES_USER=odoo \
    -e POSTGRES_PASSWORD="${POST_PASS}" \
    -e POSTGRES_DB=postgres \
    -e PGDATA=/var/lib/postgresql/data/pgdata \
    -v "${P_NAME}"-db-data:/var/lib/postgresql/data/pgdata \
    -v "${P_DIR}"/config/db-conf:/var/lib/postgresql/data/postgresql.conf \
    -p "${DB_PORT}":5432 \
    --network "${P_NAME}" \
    --name "${P_NAME}"-db --network-alias db \
    postgres:"${DB_VERSION}"
}
...
odooContainer() {
    ...
    -v "${P_DIR}"/config/odoo-conf:/etc/odoo \
    -p "$ODOO_PORT":8069 \
...
# Cloning Odoo Conf configurations from git based on requirements

confClone() {
   git clone https://gitlab.com/nazmul23/erp-odoo-conf.git \
   --depth 1 --branch "${OB}" \
   "${P_DIR}"/config/odoo-conf
}
...
enterpriseAddons() {
    ...
    "${P_DIR}"/extra-addons/enterprise-addons
}
...
while true; do
    ...
    read DB_VERSION
    echo "=================================================================="
    echo "Enter the PORT number you want DB to be exposed (DO NOT INCLUDE 5432): "
    read DB_PORT
    echo "=================================================================="
    echo "Set the Postgres Password: "
    read -s PG_PASS
    echo "=================================================================="
...
mkdir "${P_DIR}"/config/odoo-conf -p && \
mkdir "${P_DIR}"/config/db-conf && \
mkdir "${P_DIR}"/extra-addons/enterprise-addons -p
...
sed -i "s/\${ODOO_VERSION}/${ODOO_VERSION}/g" "${P_DIR}"/config/odoo-conf/odoo.conf
sed -i "s/\${ODOO_ADMIN_PASS}/${ODOO_ADMIN_PASS}/g" "${P_DIR}"/config/odoo-conf/odoo.conf
...
## Adding DB container to the monitoring environment

docker network connect docker-monitoring_default ${D_CON}
wrong "DB Container could not connect to the monitoring environment" "DB Container connected to the monitoring environment"
